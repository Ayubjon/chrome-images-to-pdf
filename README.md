...
Включить донат: впиши свой USDT (TRC-20) адрес в `DONATE_ADDRESS` (файл
`src/popup.js`) — внизу попапа появится полоска «Поддержать разработчика» с
кнопкой «Копировать». Пока стоит заглушка, полоска скрыта.
